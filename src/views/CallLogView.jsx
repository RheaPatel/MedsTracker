...
import { getCalls, addCall, deleteCall } from '../db/localStore.js'
...
export default function CallLogView({ onShare, onToast }) {
  ...
  function share(c) {
    ...
    onToast?.('Prefilled a community report — just confirm & share')
  }
  ...
  return (
    <div>
      <h1 className="view-title">Call log</h1>
      <p className="view-intro">
        Your private record of who you called and what they said. It’s the most reliable stock map
        that exists — your own recent history. Tap <strong>Share</strong> on any entry to add it to
        the community map.
      </p>
        ...
        {calls.map((c) => (
          <div key={c.id} className="card">
            <div className="card-row">
              <div className="card-title">{c.pharmacyName}</div>
            ...
            <div className="dim" style={{ fontSize: 13, marginTop: 2 }}>
              {[c.city, c.state].filter(Boolean).join(', ')} · {formatDate(c.date)}
            </div>
            {c.notes && <p className="dim" style={{ fontSize: 13, marginTop: 8 }}>{c.notes}</p>}
